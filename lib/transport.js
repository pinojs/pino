...
function isModuleReference (target) {
  return (
    target !== null &&
    (typeof target === 'object' || typeof target === 'function') &&
    typeof target.getModule === 'function'
  )
}

function resolveModuleReference (target) {
  const path = target.getModule()
  if (typeof path !== 'string') {
    throw new Error('getModule() must return a string path')
  }
  return path
}
...
function transport (fullOptions) {
  ...
  // If target is a module reference (has getModule()), resolve it to a file path
  if (isModuleReference(target)) {
    target = resolveModuleReference(target)
  }
  ...
  function fixTarget (origin) {
    // If origin is a module reference (has getModule()), resolve it to a file path
    if (isModuleReference(origin)) {
      origin = resolveModuleReference(origin)
    }
