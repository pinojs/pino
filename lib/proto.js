...
const resetChildingsSerializer = bindings => bindings
function child (bindings) {
  ...
  if (bindings.hasOwnProperty('serializers') === true) {
    ...
    for (var i = 0; i < parentSymbols.length; i++) {
      ...
      if (ks === bindingsSerializerSym) {
        instance[serializersSym][ks] = resetChildingsSerializer
      } else {
    ...
    // NOTE: this will update also the parent serializer,
    // but it shouldn't be an issue because it is called only once.
    instance[serializersSym][bindingsSerializerSym] = resetChildingsSerializer
  }
