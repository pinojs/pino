...
function child (bindings) {
  ...
  if (bindings.hasOwnProperty('formatters')) {
    instance[formattersSym] = Object.create(null)
    for (k in formatters) {
      if (k === 'bindings') {
        instance[formattersSym][k] = resetChildingsFormatter
      } else {
        instance[formattersSym][k] = formatters[k]
      }
    }
    for (k in bindings.formatters) {
      instance[formattersSym][k] = bindings.formatters[k]
    }
  } else {
    instance[formattersSym] = formatters
    // NOTE: this will update also the parent formatters,
    // but it shouldn't be an issue because it is called only once.
    instance[formattersSym].bindings = resetChildingsFormatter
  }
