...
function write (_obj, msg, num) {
  ...
  if (msg) {
    if (obj[messageKey]) {
      oldMessage = obj[messageKey]
    }
    obj[messageKey] = msg
  } else if (objError) {
    obj[messageKey] = obj.message
  } else if (obj && obj[messageKey]) {
    msg = obj[messageKey]
  }

  const s = this[asJsonSym](obj, msg, num, t, objError)

  if (oldMessage !== undefined) {
    obj[messageKey] = oldMessage
  } else {
    // This delete is not going to cause problems,
    // because the messageKey is the last property
    // added. Turbofan <3
