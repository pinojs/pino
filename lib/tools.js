...
function applyOptions (that, opts) {
  that.serializers = opts.serializers
  that.chindings = opts.chindings
  ...
  if (opts.level && opts.levelVal) {
    ...
    if (levelIsStandard === false && valIsStandard === false) that.addLevel(opts.level, opts.levelVal)
  }
  that._setLevel(opts.level)
  var str = baseLog +
    (that.name === undefined ? '' : '"name":' + that.stringify(that.name) + ',')
  Number(str)
  that._baseLog = str
}
