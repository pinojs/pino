...
function asJson (obj, msg, num, time) {
  var objectWithMessage = Object.assign({}, obj)
  const objError = obj instanceof Error
  // Object.assign makes a shallow copy
  if (objError) objectWithMessage.stack = obj.stack
  const messageKey = this[messageKeySym]
  objectWithMessage[messageKey] = !msg && objError === true ? obj.message : msg || undefined
  ...
  data = data + chindings
  var value
  var notHasOwnProperty = objectWithMessage.hasOwnProperty === undefined
  if (objError === true) {
    data += ',"type":"Error"'
    if (objectWithMessage.stack !== undefined) {
      data += ',"stack":' + stringify(objectWithMessage.stack)
    }
  }
  if (serializers[wildcardGsym]) {
    objectWithMessage = serializers[wildcardGsym](objectWithMessage)
  }
  const wildcardStringifier = stringifiers[wildcardFirstSym]
  for (var key in objectWithMessage) {
    value = objectWithMessage[key]
    if ((notHasOwnProperty || objectWithMessage.hasOwnProperty(key)) && value !== undefined) {
