...
function applyOptions (opts) {
  ...
  this.levelRules = opts.levelRules
...
// ordering function for sorting of patterns: more specific patterns
//  like 'myprefix:mylevel2:*' should be applied before 'myprefix*',
// which itself should be applied before'*'
function orderByPrecision (a, b) {
  var aix = a.indexOf('*')
  var bix = b.indexOf('*')
  if (~aix && ~bix) {
    return aix < bix ? 1 : aix === bix ? orderByPrecision(b.substr(aix+1), b.substr(bix+1)) : -1;
  } else if (~aix) {
    return 1;
  } else if (~bix) {
    return -1;
...
module.exports = {
  ...
  orderByPrecision: orderByPrecision,
  genLog: genLog
}
