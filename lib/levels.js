...
function genLsCache (instance) {
  const serializer = instance[serializersSym][levelSerializerSym]
  const { labels } = instance.levels
  const cache = {}
  Object.keys(labels).forEach(number => {
    const level = serializer(labels[number], Number(number))
    cache[number] = JSON.stringify(level).slice(0, -1)
  })
