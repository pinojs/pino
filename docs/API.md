...
+ `options` (object):
  ...
  * `messageProp` (string): the string key for the 'message' in the JSON object. Default `msg`.
...
+ `options` (object):
  ...
  * `messageProp` (string): the key in the JSON object to use as the highlighted
  message. Default: `msg`.
