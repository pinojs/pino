...
test('Native TypeScript type stripping support', { skip: skipTests ? skipMessage : false }, async (t) => {
  await t.test('pino.transport with native TypeScript file', async (t) => {
  ...
  await t.test('pino.transport with native TypeScript file URL', async (t) => {
    const destination = join(
      os.tmpdir(),
      '_' + Math.random().toString(36).substr(2, 9)
    )
  ...
  await t.test('pino.transport with multiple native TypeScript targets', async (t) => {
    const dest1 = join(
      os.tmpdir(),
      '_' + Math.random().toString(36).substr(2, 9)
    )
